...
class DatabaseManager:
    ...
    # Rooms
    def register_room(self, type, price, description, status):
    ...
    def update_room(self, room_id: int, room_type: str = None, price: float = None, description: str = None, status: str = None):
        data = {}
        if room_type:
            data["type"] = room_type
    ...
    # Bookings
    def make_booking(self, room_id, customer_id, start_date, end_date):
...
scheduler.add_job(DatabaseManager().reset_room_statuses, 'interval', hours=24)
scheduler.start()
